...
export const PanelRow =({homeTeam, awayTeam,currentRound, isLoading, index})=>{
    ...
    const editarArbitros =(equipoId: string, jugadorId: string, gol_partido: number, index, jugador_gol, jugador_name)=>{
        setIsLoadinng(true);
        let updatedGolpartido =jugador_gol += gol_partido;
        let updatedGolpartidoArr = [...homeTeam.gol_partido]; 
        updatedGolpartidoArr[index] = updatedGolpartido; 
        const formData = {gol_partido: updatedGolpartidoArr };
                            ...
                            return (
                                <>
                                    <Grid item gap={2} container alignItems={'center'} sx={{ color: light ? 'var(--dark2)' : 'var(--gris)' }}>
                                        ...
                                        <Grid item sx={{cursor:'pointer'}} onClick={()=>{editarArbitros(homeTeam._id, jugador._id,1, currentRound, jugador.gol_partido[currentRound], jugador.name)}}><Gol/></Grid>
