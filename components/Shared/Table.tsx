                ...
                <TableBody>
                    {orden.map((row,index) => (
                        <StyledTableRow key={row.id}>
                            <StyledTableCell component="th" scope="row" style={{display:'flex', gap:'8px', alignItems:'center'}}>
                                {(index + 1 == 1 || index + 1 == 2 || index + 1 == 3 || index + 1 == 4 || index + 1 == 5 || index + 1 == 6 || index + 1 == 7 || index + 1 == 8) && 
                                <Grid sx={{background:'var(--primario)', height:'35px', width:'10px'}}></Grid>}
                                {(index + 1 == 13 || index + 1 == 14 ) && 
                                <Grid sx={{background:'var(--danger)', height:'35px', width:'10px'}}></Grid>}
                                {(index + 1 == 9 || index + 1 == 10 || index + 1 == 11 || index + 1 == 12 ) && 
                                <Grid sx={{background:'var(--warnning)', height:'35px', width:'10px'}}></Grid>}
                                <Grid>{index +1}</Grid>
                                <Grid container alignItems={'center'} gap={1}><img src={row.logo} alt={row.name} style={{height:'35px'}}/> {row.name}</Grid>
                                <Grid item><ArrowP/></Grid>
                            </StyledTableCell>
