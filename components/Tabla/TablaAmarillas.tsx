...
import { jugadoresAmarillas, seleccionarData, stringAvatar } from "../../utils/utils";
import { StyledTableCell } from "../Material/StyledTableCell";
import { StyledTableRow } from "../Material/StyledTableRow";
import { useRouter } from "next/router";

export const TablaAmarillas = ({ data, isLoading, isError }) => {
        ...
        <>
            {isLoading ?
                ...
                : isError ?
                    ...
                    : jugadoresAmarillas(data, 10).length === 0 ?
                        <Grid mt={8} item sx={{ display: 'flex', flexDirection: 'row', gap: '16px', minWidth: !mobile ? '960px' : '100%', height: '500px', justifyContent: 'center', color: light ? 'var(--dark2)' : 'var(--cero)' }}>
                                ...
                                <Table aria-label="customized table">
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            <StyledTableCell light={light} align={!mobile ? "center" : "right"} style={{ whiteSpace: 'nowrap' }}>Tarjetas amarillas</StyledTableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody style={{ background: light ? 'var(--cero)' : 'var(--dark3)' }}>
                                        {jugadoresAmarillas(data, 10).map((jugador, index) => {
                                            return (
                                                <StyledTableRow light={light} key={jugador._id}>
                                                    ...
                                                    <StyledTableCell light={light} align="center" style={{ fontWeight: 700, fontSize: '15px' }}>{jugador.tarjetas_amarillas}</StyledTableCell>
                                                </StyledTableRow>
                                            )
                                        })}
                                    </TableBody>
