            ...
            <Dialog open={open} onClose={handleClose}>
                ...
                <DialogContent sx={{ background: light ? 'var(--cero)' : 'var(--dark)', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    <Grid container alignItems={'center'} gap={2}>
                        ...
                        <ButtonSend disable={false} handle={() => { editarJornada(equipoId, id, data.name, suspendido, setIsLoading, editarJornadas, queryClient, jornada, handleClose, setJornada) }} title={'Sumar'} icon={Edit} iconColor={''} iconSize={20} />
                    </Grid>
                    <Grid container alignItems={'center'} gap={2}>
                        ...
                        <ButtonSend disable={suspendido === 1} handle={() => { editarJornadaResta(equipoId, id, data.name, suspendido, setIsLoading, editarJornadas, queryClient, jornadaresta, handleClose, setJornada) }} title={'Restar'} icon={Edit} iconColor={''} iconSize={20} />
                    </Grid>
                </DialogContent>
                {isLoading && (
